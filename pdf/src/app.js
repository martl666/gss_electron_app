...
const fs = require('fs');
const options = {format: 'A4', timeout: '1000000'};
...
function createBills() {
    ...
    const allCustomerIds = customersModel.getCustomersIdForPdfDebit();
    const createdBills = [];
    allCustomerIds.forEach(async customerId => {
        ...
        await pdf.create(renderedHtml.html, options).toFile('./pdf/pdf/2024/'+renderedHtml.invoiceText+'/'+renderedHtml.invoiceNumber+renderedHtml.fileInfo+'_'+customerId.ID+'.pdf', function(err, res) {
...
function createBillsInvoice() {
    ...
    const customersModel = require('../../models/customers');
    const allCustomerIds = customersModel.getCustomersIdForPdfInvoice();
    const createdBills = [];
    allCustomerIds.forEach(async customerId => {
        let renderedHtml = new HtmlCreator(html);
        renderedHtml = await renderedHtml.getAddressBlock(customerId.ID);
...
module.exports = {
    ...
    createBillsInvoice: createBillsInvoice,
