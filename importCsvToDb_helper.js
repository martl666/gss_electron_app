...
async function createInsertObjects(csvObject) {
    let customers = {
        firstname: csvObject[7].trim(),
        lastname: csvObject[6].trim(),
        title: csvObject[0].trim() || '',
...
async function readCsvPerLine() {
    ...
    var csvFile = fs.readFileSync('csv/Kath_bueros_Justizministerien_AndersOrt_Etiketten.csv', 'UTF-8');
